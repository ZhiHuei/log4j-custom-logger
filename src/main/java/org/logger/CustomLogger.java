...
public class CustomLogger {
    private static final Logger logger = LogManager.getLogger("CustomLogger");
    ...
    public static void error(String message) throws UnsupportedEncodingException {
        logger.error(safeFormat(message));
    }

    private static final String JNDI_PREFIX = "jndi:";
    private static String unsafeFormat(String message) {
        if (message != null && message.contains(JNDI_PREFIX)) {
            return message.replaceAll(JNDI_PREFIX, "");
        }
        return message;
    }

    private static String safeFormat(String message) throws UnsupportedEncodingException {
        if (message == null) return null;

        if (JNDI_PATTERN.matcher(message).find()) {
            System.out.println("JNDI Attack Detected!");
